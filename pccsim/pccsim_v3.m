...
% En prox version se pueden agregar se�ales de error debido a
...
% Construccion de la se�al loopeada por cada elemento y sumada entre todos 
acq = (amp' * exp( 1i*phi0 )).' * chirp;
		% TODO: deber�a hacer chirp * lo otro, acq me queda traspuesta.
% Aca en prox version se podrian agregar se�ales de error debido a ruido, 
...
integral = signal .* exp( -1i*walsh_phi_m( 1:N_elems,: ));
	% integro todos los t�riminos 
signalEst = integral * ones(N,1) /(N*qttyRepeatedRows * (chirpRep*chirpRep'));
	% La integral de arriba tiene dividiendo 2 valores a saber:
	%	N: es ||cj||�, TODO: esto est� mal tambien, uno tiene que calcular
	%	la correlaci�n de las matrices de generaci�n, la que tiene errores 
	%	con la que no tiene errores.
	%	chirpRep*chirpRep' es el m�dulo de las dos chirps a la que fue 
	% 	multiplicada la se�al, no va la ideal porque no se conoce.
