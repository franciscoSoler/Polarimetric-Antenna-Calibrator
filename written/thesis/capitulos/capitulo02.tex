...
otro sin calibrar. Se puede observar como los niveles de los lóbulos secundarios, el ancho del lóbulo principal se incrementan y
el nivel del lóbulo principal decrementa.
\begin{figure}[H]
 \centering
 \includegraphics[width=10cm]{gfx/nonCalPattern.png}
 \caption{Diagrama de radiación de antena calibrado (en negro) vs no calibrado (en gris).}
...
El acoplamiento mutuo entre elementos radiantes se encuentra definido en \cite{Dawoud2005}. Dado que el efecto sobre la señal
transmitida en los lazos de calibración es eliminado, no es necesario que dicha modelización sea exacta.

La modelización utilizada es la misma que la de un cable para el desfase de la señal, pero con las propiedades de atenuación 
de una onda en \enquote*{espacio libre} \cite{Curlander1991}.

\begin{equation}
	S12 = S21 = \dfrac{\lambda e^{-j\frac{2\pi}{\lambda}r}}{4\pi r}
\end{equation}

siendo r la distancia recorrida por la señal. La matriz de parámetros S resulta,

$$
\mathbf{S} = \begin{pmatrix} 0 & \frac{\lambda e^{-j\frac{2\pi}{\lambda}r}}{4\pi r}\\\frac{\lambda e^{-j\frac{2\pi}{\lambda}r}}{4\pi r} & 0\end{pmatrix}
$$
