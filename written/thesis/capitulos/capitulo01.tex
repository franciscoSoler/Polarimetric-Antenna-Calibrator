...
La máxima transferencia de potencia requiere que la impedancia de un sistema de
...
Es un instrumento que mide los parámetros eléctricos de una red. Hoy, dichos analizadores miden los parámetros S porque las
...
dos puertos \cite{NetworkAnalyzer}. En la figura \ref{fig:tipicalVNA} muestra el esquema de conexionado típico para caracterizar
cualquer circuito con un VNA.
...
Es definida como la estructura transicional enre espacio libre y un dispositivo guía, como se muestra en la 
...
elemento radiante posee un defasador y que, dependiendo de la fase de cada uno, el apuntamiento de la antena se puede modificar.
Los defasadores se encuentran controlados por una unidad central de control.

\begin{figure}[H]
 \centering
 \includegraphics[width=10cm]{gfx/beamSteering.png}
 \caption{Apuntamiento del haz controlado por los defasadores \cite{BeamSteering}.}
\end{figure}
...
son facilmente accesibles y/o que requieren una calibracion frecuente.

Para antenas que vuelan por ejemplo en satelites, las mediciones obtenidas con este sistema son útiles solamente al utilizarlas
...
separación de $0.2m$ entre ellos. La frecuencia central emitida es de 1275MHz y cada ensayo se realiza tres veces apuntando cada
...
y la densidad de potencia radiada utilizando un radiador isotrópico, el cual emite uniformemente en toda dirección
\cite{Standard1996}.
...
huella es la traza representada por la intersección del lobulo principal con la tierra. La figura \ref{fig:huellaSatellite}
...
\subsubsection{La matríz de parámetros S}
...
impedancias resultan en ondas estacionarias a lo largo de dicha línea de transmisión. El SWR está definido como la relación
entre la amplitud de la onda reflejada y la amplitud de la onda incidente \cite{swrWiki}.
...
\todo{pedir a ezequiel el libro de control 1 para el grafico de sistema de control de lazo cerrado}
...
cerrado. A partir de sensados, se envia la informacion de fase y atenuacion a los modulos transmisores y receptores para obtener
...
Para obtener el efecto de una antena sintética, la misma es transadada tomando posiciones secuenciales a lo largo de una línea. 
