...
def is_psc(component):
    return re.match("PSC1.*", component) if isinstance(component, str) else False
...
def get_qtty_ports(component):
    if re.match("PSC1(.*)|cable|RM|TRM|circulator", component) is None:
        raise Exception("the input component({0}) is not from the network", component)

    match = re.match("PSC1(.*)|cable|RM", component)
