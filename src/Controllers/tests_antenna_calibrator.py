...
class MyTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.calibrator = AntennaCalibrator.MutualCalibrator(self.power, self.phase, self.row_steering, self.column_steering, self.separation, self.separation,
                                                             self.filename)
    ...
    def test_antenna_calibrator_retrieve_the_correct_gain(self):
        ...
        np.testing.assert_almost_equal(tx_power, common.v2db(np.abs(tx_network[:, :, 1, 0]) * common.db2v(self.power)))
